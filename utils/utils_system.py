...
# PyTorch Lightning 2.x compatibility: recursive_gather and recursive_stack removed
try:
    from pytorch_lightning.core.step_result import recursive_gather, recursive_stack
except ImportError:
    # 为兼容性提供简单的实现
    def recursive_gather(data, group=None):
...
def build_base_encoder(
    ...
    elif backbone.lower() == "vggish":
        # VGGish backbone for audio data
        from models.vggish_model import VGGish
        import os
        pretrained_path = os.path.join("models", "vggish-10086976.pth") if pretrained else None
        # Create VGGish without classification head for feature extraction
        encoder = VGGish(num_classes=2)
        if pretrained_path and os.path.exists(pretrained_path):
            try:
                state_dict = torch.load(pretrained_path, map_location='cpu')
                if 'state_dict' in state_dict:
                    state_dict = state_dict['state_dict']
                # Load only the feature layers
                model_dict = encoder.state_dict()
                pretrained_dict = {k: v for k, v in state_dict.items() if k.startswith('features.') and k in model_dict}
                if pretrained_dict:
                    model_dict.update(pretrained_dict)
                    encoder.load_state_dict(model_dict, strict=False)
                    print(f"Loaded {len(pretrained_dict)} pretrained VGGish parameters")
            except Exception as e:
                print(f"Warning: Could not load VGGish pretrained weights: {e}")
        # Remove the final classification head, keep only feature extraction
        encoder.head = nn.Identity()
        encoder.final_feat_dim = 128  # VGGish embedding dimension
