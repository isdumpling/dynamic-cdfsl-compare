...
echo "----------------------------------------"
echo "使用ResNet10 backbone在ColdZone数据集上进行预训练..."

if [ ! -f "ckpt/ce_audio_ColdZone_resnet10/last.ckpt" ]; then
    python main.py system=audio_ce \
      ...
      ++data.n_way=2 \
      ++n_way=2 \
      ++model_name=ce_audio_ColdZone_resnet10 \
      ++trainer.gpus=1 \
    ...
    echo "如需重新训练，请删除: ckpt/ce_audio_ColdZone_resnet10/"
fi
...
echo ""
echo "模型保存位置："
echo "  - Stage 1: ckpt/ce_audio_ColdZone_resnet10/last.ckpt"
echo "  - Stage 2: ckpt/dynamic_cdfsl_audio_HotZone_resnet10/last.ckpt"
echo ""
echo "进行Few-shot评估："
echo "  python main.py system=few_shot \\"
echo "    ++backbone=resnet10 \\"
echo "    ++data.test_dataset=HotZone \\"
echo "    ++data.n_way=2 \\"
echo "    ++data.n_shot=5 \\"
echo "    ++data.n_query=15 \\"
echo "    ++ckpt=ckpt/dynamic_cdfsl_audio_HotZone_resnet10/last.ckpt \\"
echo "    ++test=true"
echo ""
