...
class LearningRateMonitor(Callback):
    ...
    def on_train_start(self, trainer, pl_module, *args, **kwargs):
        ...
        # 兼容新旧版本获取lr_schedulers
        lr_schedulers = getattr(trainer, 'lr_schedulers', None)
        if lr_schedulers is None:
            # 新版本可能在lr_scheduler_configs中
            ...
            def _check_no_key(key):
                ...
                for sch in lr_schedulers:
                    if isinstance(sch, dict):
                        optimizer = sch['scheduler'].optimizer
                    else:
                        optimizer = sch.scheduler.optimizer
                    result.append(key not in optimizer.defaults)
                return any(result)
        ...
        names = self._find_names(lr_schedulers)
    ...
    def _extract_stats(self, trainer, interval: str) -> Dict[str, float]:
        ...
        for name, scheduler in zip(self.lr_sch_names, lr_schedulers):
            # 兼容新旧版本：scheduler可能是dict或LRSchedulerConfig对象
            if isinstance(scheduler, dict):
                sched_interval = scheduler['interval']
                sch = scheduler['scheduler']
            else:
                # 新版本使用LRSchedulerConfig对象
                sched_interval = scheduler.interval
                sch = scheduler.scheduler
                
            if sched_interval == interval or interval == 'any':
                opt = sch.optimizer
    ...
    def _find_names(self, lr_schedulers) -> List[str]:
        ...
        for scheduler in lr_schedulers:
            ...
            if isinstance(scheduler, dict):
                ...
                # 新版本使用LRSchedulerConfig对象
                sch = scheduler.scheduler
                sched_name = getattr(scheduler, 'name', None)
