...
# string_classes was (str, bytes) in Python 3
...
class MultiTrainLoaderMixin(object):
    ...
    def build_unlabel_loader(self):
        ...
        for dset in datasets:
            ...
            # 检查是否使用DDP（兼容新旧版本）
            use_ddp = False
            if hasattr(self.trainer, 'use_ddp'):
                use_ddp = self.trainer.use_ddp
            elif hasattr(self.trainer, 'strategy'):
                strategy_name = str(type(self.trainer.strategy).__name__).lower()
                use_ddp = 'ddp' in strategy_name
